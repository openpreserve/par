 ...
 * <p>
 * Synchronous and Asynchronous calls for all methods are provided for here.
 * <p>
 * See {@link org.parcore.api.client.endpoints.PreservationActionsApiImpl}
 * or {@link org.parcore.api.client.endpoints.PreservationActionTypesApiImpl}
...
public class AbstractApiCallImpl<W, E> {
    ...
    /**
     * Returns an empty map if there are no values in the list, or a singleton map containing an entry keyed by the key
     * and with an ampersand separated list of the values parameter as the value
     *
     * @param key    Header Key to use
     * @param values The list of values to use as filters
     * @return key value pair
     */
    protected Map<String, String> getFilterHeader(String key, List<String> values) {
        if (values == null) {
            throw new IllegalArgumentException("List of filter values must not be null, pass an empty list instead (e.g. Collections.emptyList()");
        }
        String value = StringUtil.join((String[]) values.toArray(), "&");
        return ("".equals(value)) ? Collections.emptyMap() : Collections.singletonMap(key, value);
    }
}
